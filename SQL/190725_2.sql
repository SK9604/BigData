...
select * from user_users; --사용자 정보 조회
select * from tabs; --현재 사용자 소유의 테이블 정보 조회.
select * from dba_data_files; --system 계정으로 접속해야 조회가능
select * from dba_tablespaces;  --tablespace에 대한 상세한 정보 조회
...
create table test_tbs(
    txt varchar2(1000)
) tablespace my_tbs;    --tablespace를 명시적으로 지정
...
drop tablespace my_tbs including contents and datafiles cascade constraints; --tablespace 파일 삭제
select * from dba_data_files;
