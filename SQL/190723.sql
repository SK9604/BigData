...
--emp_copy
create or replace procedure delete_emp_by_dept_id_sp (p_dept_id dept.dept_id%type)
is
...
/
select * from emp where dept_id in (110, 100);
...
--        fetch dept_cur into rec_dept.dept_id;
--        fetch dept_cur into v_id, v_name, v_loc;
...
begin
    open emp_cur;
    loop
        fetch emp_cur into rec_emp.emp_id, rec_emp.emp_name, rec_emp.salary, rec_emp.comm_pct;
        exit when emp_cur%notfound;
        dbms_output.put_line(rec_emp.emp_id||'-'||rec_emp.emp_name||'-'||rec_emp.salary||'-'||rec_emp.comm_pct);
    end loop;
    close emp_cur;        
end;
/
...
declare
    cursor dept_cur(cp_loc dept.loc%type) is
        ...
        where loc = cp_loc;
begin
    for dept_row in dept_cur('Seattle') -- for문에서 커서를 open
    loop
...
begin 
    ...
    loop
        dbms_output.put_line(e_row.emp_id||'-'||e_row.emp_name||'-'||e_row.salary||'-'||e_row.comm_pct);
    end loop; 
end;
/
...
begin
    ...
    loop
--        dbms_output.put_line(d_row.dept_id);
        update dept_copy
        set loc = '인천'
...
create or replace procedure todo06_01(p_job_id job.job_id%type, p_rate binary_double)
is
    v_max_salary job.max_salary%type;--업무의 max_salary를 저장할 변수
    cursor emp_cur is
        select  emp_id
        from    emp
...
exec todo06_01('AD_PRES', 0.2);
select * from emp where job_id = 'AD_PRES';
-- TODO: EMP테이블에서 커미션(comm_pct)이 NULL이 직원의 ID(emp_id), 이름(emp_name) , 급여(salary), 커미션(comm_pct)를 이름 오름차순으로 정렬한 결과를 출력하는 stored Procedure를 구현
...
is
begin
    for e_row in (select emp_id, emp_name, salary, hire_date from emp
...
--TODO: 부서의 이름(dept.dept_name)을 파라미터로 받아서 그 부서에 속한 직원의 ID(emp.emp_id), 직원이름(emp.emp_name), 급여(emup.salary)를 급여가 많은 순으로 출력하는 stored Procedure를 구현.  
create or replace procedure EX_06_02(p_dept_name dept.dept_name%type)
is
...
create or replace procedure EX_06_03(p_num binary_integer)
is
    cursor dept_id_cur is
        select dept_id
        from emp
...
커미션 내역 테이블
********************************** */
drop table commission_trans;
create table commission_trans(
    emp_id number(6), 
    commission number,
    comm_date date
);
-- TODO: EMP 테이블에서 COMM_PCT 가 있는(not null) 직원의 커미션(salary * comm_pct)을 commission_trans 테이블에 저장하는 
--stored Procedure를 구현.
...
begin
    for e_row in emp_cur
    loop
        insert into commission_trans values (e_row.emp_id, e_row.salary * e_row.comm_pct, sysdate);
    end loop;
    commit;
end;
/
