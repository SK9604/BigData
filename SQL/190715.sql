...
select '최대값' as Label, max(salary)as "집계결과" from emp   --첫번째 컬럼인 max만 결과 테이블 컬럼에 보여짐
union all                                                   --union : 행을 늘림
...
select * from emp where dept_id in(10,20)
minus --차집합: 첫번째 조회결과에서 두번째 결과에 없는 것만 조회
select * from emp where dept_id in(20,30);

select * from emp where dept_id in(10,20)
intersect --교집합: 두 조회결과에 모두 있는 것들만 조회
select * from emp where dept_id in(20,30);
...
select * from customers             --전체고객
minus                               --주문한 고객 빼기
select * from customers where cust_id in (select cust_id from orders); --주문한 고객
...
select '수출' , item from export_rank where year = 1990
union all
...
minus
select item from export_rank where year = 2000;
--select item from export_rank where year in (2018, 2000);
